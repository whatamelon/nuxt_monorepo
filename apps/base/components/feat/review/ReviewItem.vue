<template>
  <div class="flex w-full space-x-2 pt-4">
    <div v-if="reviewItem.imgs != '' && reviewItem.imgs != 'none' && reviewItem.imgs != null" class="relative h-22 w-22" @click="openReviewImgList">
      <NuxtImg :src="reviewItem.imgs.split(',')[0]" fit="cover" class="h-full w-full object-cover" alt="리뷰 이미지" />
    ...
    <div v-if="reviewItem.imgs != '' && reviewItem.imgs != 'none' && reviewItem.imgs != null" class="inline-block max-h-[88px] w-[calc(100%-88px)]">
      ...
      <p class="body-1-r line-clamp-3 break-words break-keep text-black">{{ reviewItem.msg }}</p>
    </div>

    <div v-else class="inline-block max-h-[88px] w-full">
      <div class="mb-1.5 flex h-fit justify-between">
        <div class="flex space-x-1">
