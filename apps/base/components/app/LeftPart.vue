<template>
  <ClientOnly>
    ...
    >
      <NuxtImg
        :src="getLayoutObject.logoImage"
        class="mt-10 h-10 cursor-pointer"
        :class="{
          'h-3': $device.isTablet,
          'h-6': $device.isDesktop
        }"
        :alt="getLayoutObject.titleTxt + ' 로고'"
        @click="goHome"
      />
      <div class="absolute top-1/2 h-fit w-full" style="transform: translateY(-50%)">
        <h1 class="headline-b" :style="setTextColor(getLayoutObject.titleColor)">{{ getLayoutObject.titleTxt }}</h1>
        <p class="body-2-r mt-1 opacity-60" :style="setTextColor(getLayoutObject.bodyColor)">{{ getLayoutObject.bodyTxt }}</p>

        <div class="mt-10">
          <div v-if="getLayoutObject.cardList.length > 1" class="leftpart-swiper">
            <Swiper
              ...
              :effect="'fade'"
            >
              <SwiperSlide v-for="(layoutBanner, bannerIdx) in getLayoutObject.cardList" :key="bannerIdx">
                <div
          ...
          <div v-else>
            <div
              :style="setBgColor(getLayoutObject.cardList[0].bgColor)"
              class="flex flex-col justify-between rounded-3xl"
              :class="{
                'h-[340px] w-[280px] px-6 py-8': $device.isTablet,
                'h-[460px] w-[320px] px-8 py-10': $device.isDesktop
            ...
            >
              <div>
                <NuxtImg
                  :src="getLayoutObject.cardList[0].imgLink"
                  class="mx-auto w-fit"
                  :class="{
                    'h-20': $device.isTablet,
                    'h-30': $device.isDesktop
                  }"
                  :alt="getLayoutObject.cardList[0].titleTxt"
                />
                <h2
                  ...
                  :style="setTextColor(getLayoutObject.cardList[0].titleColor)"
                >
                  {{ getLayoutObject.cardList[0].titleTxt }}
                </h2>
                <h3
                  ...
                  :style="setTextColor(getLayoutObject.cardList[0].bodyColor)"
                >
                  {{ getLayoutObject.cardList[0].bodyTxt }}
                </h3>
              </div>
              ...
              <button
                ...
                :style="setBgTextColor(getLayoutObject.cardList[0])"
                @click="() => move(getLayoutObject.cardList[0])"
              >
                {{ getLayoutObject.cardList[0].buttonTxt }}
              </button>
            </div>
          </div>
        </div>
...
// getLayoutObject 로부터 데이터를 받아옵니다.
...
const { getLayoutObject } = storeToRefs(appStore())
