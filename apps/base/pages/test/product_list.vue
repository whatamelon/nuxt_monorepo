<template>
  <div>
    <div class="sticky top-14 z-50">
      <PartProductListCategoryOneList v-show="isShowTab" @update-cat="onUpdateCategoryGender"></PartProductListCategoryOneList>
...
const onUpdateCategoryClothes = (cat) => {
  amplitude.track('filter', {
    ...
    category: formatAmpCatNameFilter(cat)
  })
...
const onSetQueryFilter = () => {
  ...
  if (route.query.type) {
    const queryList = route.query.type.split(',')

    queryList?.forEach((queryItem) => {
      typeFilterList.value.push(getTypeList.value.find((item) => queryItem === item.code))
    })
  }
...
/**
 * 카테고리 필터 우선순위 List 세팅 (도메인 카테고리, 성별 카테고리, 탭 카테고리)
